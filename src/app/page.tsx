...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    // Uses selective projection and SWR caching to minimize network payload
    Promise.all([
      fetchWithCache('home_rovers', () => clientSql`SELECT id, slug, name, year, competition, rank_achieved, cover_image, tagline, specs FROM rovers ORDER BY year DESC LIMIT 5;`),
      fetchWithCache('home_achievements', () => clientSql`SELECT id, year, title, competition, rank, location, image_url, description, is_featured FROM achievements WHERE is_featured = true ORDER BY year DESC LIMIT 4;`),
      fetchWithCache('home_events', () => clientSql`SELECT id, title, event_date, location, image_url, description, category, status FROM events ORDER BY id ASC LIMIT 3;`),
      fetchWithCache('home_media', () => clientSql`SELECT id, publisher_name, headline, article_url, publish_date, clipping_image, category FROM media_articles ORDER BY id ASC LIMIT 8;`),
      fetchWithCache('home_sponsors', () => clientSql`SELECT id, name, logo_url, tier, website_url FROM sponsors ORDER BY id ASC;`),
      fetchWithCache('home_content', () => clientSql`SELECT key, value, section, type FROM site_content;`),
    ])
