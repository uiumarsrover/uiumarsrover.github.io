...
export default function RoverDetailClient({ initialRover, slug }: { initialRover: any; slug: string }) {
  const [rover, setRover] = useState<any>(initialRover);

  useEffect(() => {
    clientSql`SELECT * FROM rovers WHERE slug = ${slug} LIMIT 1;`
      .then((res: any) => {
        if (res && res.length > 0) {
          setRover(res[0]);
        }
      })
      .catch((err) => console.error('Real-time rover detail sync error:', err));
  }, [slug]);
