...
export default function RoversPage() {
  ...
  const [isLoading, setIsLoading] = useState<boolean>(!rovers || rovers.length === 0);

  useEffect(() => {
    fetchWithCache('rovers_list', () => clientSql`SELECT id, slug, name, year, competition, rank_achieved, cover_image, tagline, description, specs FROM rovers ORDER BY year DESC;`)
      .then((res: any) => {
