    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/10 pb-4">
        ...
        <span className="text-xs font-mono text-gray-400">
          Showing <strong>{Math.min(visibleCount, filteredItems.length)}</strong> of <strong>{filteredItems.length}</strong> Press Clippings
        </span>
      </div>
      ...
      {filteredItems.length > visibleCount && (
        <div className="text-center pt-6">
          <button
            ...
            className="px-8 py-3 rounded-2xl bg-white/5 hover:bg-white/10 text-white font-mono text-xs uppercase font-bold border border-white/15 hover:border-mars-500/50 transition shadow-lg inline-flex items-center gap-2"
          >
            <span>Show More Clippings ({filteredItems.length - visibleCount} remaining)</span>
            <Sparkles className="w-4 h-4 text-mars-400" />
          </button>
        </div>
      )}
